...
class ExperimentLogger:
    ...
    def log_population(self, population):
        for p in population:
            self.log_data(self.attempt, p.name, p.data)
    ...
    def log_data(self, attempt, name, data):
        """Logs the HIIT workout data to a file."""
        with open(
            f"{self.dirname}/code/try-{attempt}-{name}.json", "w", encoding="utf-8"
        ) as file:
            # file.write(text)
            json.dump(convert_to_serializable(data), file, indent=2)
        self.attempt = attempt
